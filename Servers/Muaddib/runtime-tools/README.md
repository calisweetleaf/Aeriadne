...
## 8. Security, data-lane, and transport hardening addendum (2026-06-06)

- Live/source parity counts are dated evidence only. Do not copy tool counts without a fresh source probe plus `bb7_tool_health_report` after the target gateway has loaded.
- Tool-usage SQLite capture is a substrate lane, not a public transcript. `mcp_server.py` redacts sensitive argument keys and bearer/API-key-like strings before persistence; `SOVEREIGN_TOOL_USAGE_DB_ENABLED=0` disables that DB lane if required.
- Preformatted MCP `content` payloads still carry display-projection metadata so downstream consumers can distinguish display passthrough from raw substrate truth.
- Async tool futures are bounded by `SOVEREIGN_ASYNC_TOOL_TIMEOUT_SECONDS`; timeouts are recorded as internal failures instead of silently hanging the gateway.
- HTTPS transport defaults are conservative: API keys are redacted from stdout/logs unless explicitly revealed once, query-parameter API keys are disabled by default, forwarded headers are trusted only from configured proxy CIDRs, and non-loopback binding requires explicit external authorization.
- Webhook registration validates callback URLs before persistence and before delivery. HTTP, localhost/private/link-local/reserved destinations, URL credentials, and non-allowlisted hosts are rejected unless explicitly enabled by environment flags.
- OpenRouter wrapper health/dry-run remains no-network. Live OpenRouter calls require a real `OPENROUTER_API_KEY`; distillation capture is bounded, redacted, flushable, and shut down explicitly.

...
Fires on every Codex session boot. Codex's matcher distinguishes startup /
resume / clear — Mentat treats all three the same way for FSA purposes
(SESSION_START transitions to PLANNING from any state) but tags the matcher
in the insight payload so the debrief skill can correlate.

If a handoff.md exists from a prior compaction or stop, prepend it to the
next turn via additionalContext so the model sees its own prior state on
resume.
"""
...
def main() -> int:
    ...
    ctx.step(Event(event_class=EventClass.SESSION_START,
                   payload={"matcher": matcher, "runtime": "codex"}))
    ...
    if handoff_text:
        ctx.bus.emit(Insight(
            ...
            state=ctx.session.state.value,
            payload={"bytes": len(handoff_text), "runtime": "codex"},
        ))
        msg = (
            "<mentat:handoff>\n"
